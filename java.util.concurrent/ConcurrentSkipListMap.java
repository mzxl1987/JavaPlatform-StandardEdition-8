...
public class ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
     ...
     * messy at best.)
     *
     * Here's the sequence of events for a deletion of node n with        删除n(前节点p,后续节点f)节点的顺序事件
     ...
     *        +------+       +------+      +------+
     *
     * 1. CAS n's value field from non-null to null.                     利用CAS方法,将n节点的value由non-null设置为null
     *    From this point on, no public operations encountering          没有公共操作认为该节点在Map中存在
     *    the node consider this mapping to exist. However, other        其他的插入/删除操作可能仍然会修改n的next引用 
     *    ongoing insertions and deletions might still modify
     *    n's next pointer.
     *
     * 2. CAS n's next pointer to point to a new marker node.            利用CAS方法,将n.next引用指向一个新的mark节点
     *    From this point on, no other nodes can be appended to n.       没有其他节点紧接在n节点的后面
     *    which avoids deletion errors in CAS-based linked lists.        同时也避免了 基于CAS操作链表产生的错误
     *
     *        +------+       +------+      +------+       +------+
     *   ...  |   b  |------>|   n  |----->|marker|------>|   f  | ...
     *        +------+       +------+      +------+       +------+
     *
     * 3. CAS b's next pointer over both n and its marker.                利用CAS操作,修改b.next节点,跨过n,marker节点,指向f节点
     *    From this point on, no new traversals will encounter n,         这时，就没有遍历能够遇到n节点了
     *    and it can eventually be GCed.                                  n节点最终被GC回收
     *        +------+                                    +------+
     *   ...  |   b  |----------------------------------->|   f  | ...
     *        +------+                                    +------+
     *
    ...
    static final class Node<K,V> {
        final K key;                                                      // Key : map中的Key
        volatile Object value;                                            // 节点的值
        volatile Node<K,V> next;                                          // 链表下一个节点的索引
         ...
         */
        boolean casNext(Node<K,V> cmp, Node<K,V> val) {                        // 利用nextOffset偏移量可以找到,this.next引用
            return UNSAFE.compareAndSwapObject(this, nextOffset, cmp, val);    // 利用CAS比较交换, this.next == cmp ? this.next = val : doNothing;
        }
        ...
        void helpDelete(Node<K,V> b, Node<K,V> f) {
             ...
             */
            if (f == next && this == b.next) {                           //判断n介于f,b之间, b.next = n, n.next = f
                                                                         // 如果当前节点被标记, n.next = f = new Node<K,V>(f) = marker
                                                                         // marker节点的特点参照 new Mode<K,V>()构造函数
                                                                         // marker.value = this; marker.next = f, marker.key = null;
                if (f == null || f.value != f) // not already marked     
                    casNext(f, new Node<K,V>(f));                        // 插入marker节点 ， b > n > marker > f
                else
                    b.casNext(this, f.next);                             // 如果被标记删除了,则直接跨过n. from b > n > marker > f to b > f
            }
        }
     ...
     */
    static class Index<K,V> {                                         // 跳跃表的索引,索引中重要的3属性
        final Node<K,V> node;                                         // 1. Node<K,V> 节点引用，不可更改
        final Index<K,V> down;                                        // 2. down, 表示当前索引的向下的索引
        volatile Index<K,V> right;                                    // 3. right, 表示当前索引的向右索引
        ...
        /**
         * Tries to CAS right field to skip over apparent successor        //利用CAS将 跳过 successor
         * succ.  Fails (forcing a retraversal by caller) if this node     //如果当前节点已经被标记为删除,则失败
         * is known to be deleted.                
         * @param succ the expected current successor                      // from  this > successor > successor.right
         * @return true if successful                                      // to    this > successor.right
         */
    ...
    private Node<K,V> findPredecessor(Object key, Comparator<? super K> cmp) {
        ...
        for (;;) {
            for (Index<K,V> q = head, r = q.right, d;;) {
                if (r != null) {                                    //当right索引不为空时
                    Node<K,V> n = r.node;                           //获得right.Node节点
                    K k = n.key;                                    //获得node.Key
                    if (n.value == null) {                          //如果node.Value是null
                        if (!q.unlink(r))                           //尝试跨过right
                            break;           // restart             //skip 失败,重新开始
                        r = q.right;         // reread r            //skip 成功,重新读取right索引
                        continue;                                   //continue
                    }
                    if (cpr(cmp, key, k) > 0) {                     //比较key,right.node.key的大小
                        q = r;                                      // > 0,索引继续向右移动
                        r = r.right;
                        continue;                                   // continue
                    }
                }
                if ((d = q.down) == null)                           //down索引下移,并判断是否为null
                    return q.node;                                  //为null,返回当前节点
                q = d;
                r = d.right;
            }
        }
    ...
    private Node<K,V> findNode(Object key) {
        ...
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {   // n是key要找的节点
                Object v; int c;
                if (n == null)
                    break outer;
                ...
                if (n != b.next)                // inconsistent read         // 读取不一致
                    break;
                if ((v = n.value) == null) {    // n is deleted               // n已被删除
                    n.helpDelete(b, f);                                       // 删除n节点
                    break;
                }
                if (b.value == null || v == n)  // b is deleted                //b节点也被删除,此时的n是mark节点;因为 n.value = n,具体原因参看Node()构造函数
                    break;                                                     //退出当前循环
                if ((c = cpr(cmp, key, n.key)) == 0)                           //将key & n.key进行比较, == 0 表示相等
                    return n;                                                  //返回n节点
                if (c < 0)
                    break outer;
                b = n;
     ...
     */
    private V doGet(Object key) {                                          // doGet 与 findNode()基本相同
        if (key == null)
        ...
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                if (n != null) {
                    ...
                    if (c == 0) {                                           // key已经存在
                        if (onlyIfAbsent || n.casValue(v, value)) {         // onlyIfAbsent ? 直接返回value : 更新值&返回value
                            @SuppressWarnings("unchecked") V vv = (V)v;
                            return vv;
                        }
                ...
                z = new Node<K,V>(key, value, n);                           // 创建新节点z, z.next = n
                if (!b.casNext(n, z))                                       // 将z插入b,n之间, from b > n to b > z > n
